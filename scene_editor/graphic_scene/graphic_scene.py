...
class GraphicScene(QtGui.QGraphicsScene):
    ...
    def grid_point(self, scene_pos):
        '''Возвращает координаты прикреплённые к сетке, в зависимости от флага use_grid_point'''
        ...
        if x_mod > 0.5:
            res.setX((abs(scene_pos.x()) + (1.0 - x_mod) * self.GRID_STEP) * x_sign)
        else:
        ...
        if y_mod > 0.5:
            res.setY((abs(scene_pos.y()) + (1.0 - y_mod) * self.GRID_STEP) * y_sign)
        else:
    ...
    def scene_grid_steps(self, view):
        ...
        x_step = (self.GRID_PIXEL_STEP / view.viewport().width()) * scene_rect.width()
        x_step -= math.modf(x_step / self.GRID_STEP)[0] * self.GRID_STEP
        
        y_step = (self.GRID_PIXEL_STEP / view.viewport().height()) * scene_rect.height()
    ...
    def draw_backgroud_grid(self, view, painter, rect):
        ...
        top = rect.top() - math.modf(rect.top() / grid_step[1])[0] * grid_step[1]
    ...
    def draw_foreground_grid(self, view, painter, rect):
        ...
        painter.setPen(self.RULER_PEN)
        painter.setBrush(self.RULER_BRUSH)
        ruler_rect = QtCore.QRectF(0, self.RULER_WIDTH, self.RULER_WIDTH, height - self.RULER_WIDTH)
        ...
        top = rect.top() - math.modf(rect.top() / grid_step[1])[0] * grid_step[1]
        offset = self.RULER_WIDTH / 2.0 - QtGui.QFontMetrics(self.GRID_FONT).height() / 2.0       
        #offset = 100
