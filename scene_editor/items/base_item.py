...
class BaseItem(QtGui.QGraphicsItem):
    ...
    def __init__(self, **kw_args):
        ...
        self.__attach_grid = kw_args[
            "attach_grid"] if "attach_grid" in kw_args else True
        super(BaseItem, self).__init__()
        self.setFlag(QtGui.QGraphicsItem.ItemIsMovable, True)
        self.setFlag(QtGui.QGraphicsItem.ItemIsSelectable, True)
        self.setFlag(QtGui.QGraphicsItem.ItemSendsGeometryChanges, True)

        self.__context_menu = kw_args[
            "context_menu"] if "context_menu" in kw_args else None
        self.__rect = (
            kw_args["rect"] if isinstance(kw_args["rect"], QtCore.QRectF)
            else QtCore.QRectF(
                kw_args["rect"][0],
                kw_args["rect"][1],
                kw_args["rect"][2],
        ...
        self.__editable = kw_args[
            "editable"] if "editable" in kw_args else True
        self.__edit_marker_size = kw_args[
            "edit_marker_size"] if "edit_marker_size" in kw_args else 10

        # self.setPos((
        #     kw_args["pos"] if isinstance(kw_args["pos"], QtCore.QPointF)
        #     else QtCore.QPointF(kw_args["pos"][0], kw_args["pos"][1]))
    ...
    def mouseMoveEvent(self, event):
        if self.__cur_edit_marker is not None:
            ...
            self.__cur_edit_marker[2](r, self.itemChange(
                self.CORNER_CHANGED, QtCore.QVariant(event.pos() + self.offset)).toPointF())
            self.set_rect(r)
            return
