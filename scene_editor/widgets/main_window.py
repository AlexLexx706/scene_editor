...
from scene_editor.items.brick_item import BrickItem
from scene_editor.items.robot_item import RobotItem
from scene_editor.items.shit_item import ShitItem
...
class MainWindow(QtGui.QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        uic.loadUi(os.path.join(
            os.path.split(__file__)[0], "main_window.ui"), self)
        ...
        self.listView_objects.selectionModel().selectionChanged.connect(
            self.on_listView_objects_selectionChanged)
    ...
    def drop_new_object(self, desc):
        if desc["type"] != -1:
            ...
            if desc["type"] == BrickItem.item_type():
                self.items_model.add_item(BrickItem(
                    context_menu=self.menu_elements,
                    rect=QtCore.QRectF(-20, -20, 40, 40),
                    name=u"Стена"), pos=desc["scene_pos"])
            elif desc["type"] == RobotItem.item_type():
                self.items_model.add_item(RobotItem(
                    context_menu=self.menu_elements,
                    rect=QtCore.QRectF(-20, -20, 40, 40),
                    name=u"Бендер"), pos=desc["scene_pos"])
            elif desc["type"] == ShitItem.item_type():
                self.items_model.add_item(ShitItem(
                    context_menu=self.menu_elements,
                    rect=QtCore.QRectF(-10, -10, 20, 20),
                    name=u"Хоу-ноу"), pos=desc["scene_pos"])
    ...
    def on_scene_selectionChanged(self):
        ...
        self.listView_objects.setCurrentIndex(
            self.items_model.get_index(items[0]))
    ...
    def on_action_load_triggered(self, checked):
        file_path = QtGui.QFileDialog.getOpenFileName(
            self, u"Загрузить файл модели", "", "*.json")
        ...
        if len(file_path):
            ...
            with open(file_path, "rb") as f:
                for desc in json.loads(f.read()):
                    self.items_model.add_item(BaseItem.create(
